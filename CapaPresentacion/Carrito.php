...
class Carrito {
	...
	private function update_precio_cantidad() {
		...
		// refrescamos el contenido del carrito para que quede actualizado
		$this->update_carrito ();
	}
	
	// método que actualiza la cantidad
	// de productos de un producto del carrito
	public function update_cantidad($id, $cantidad) {
		...
		if (! empty ( $this->carrito )) {
			foreach ( $this->carrito as $row ) {
				...
				if ($row ["unique_id"] === $id) {
					...
					$articulo = $_SESSION ["carrito"] [$id];
					$articulo ["cantidad"] = trim ( preg_replace ( '/([^0-9\.])/i', '', $cantidad ) );
					$articulo ["total"] = $articulo ["cantidad"] * $articulo ["precio"];
					// /ahora añadimos el producto al carrito
					$_SESSION ["carrito"] [$id]= $articulo;
				}
			}
		}
		...
		// actualizamos el precio total y el número de artículos del carrito
		// una vez hemos actualizado el producto
		$this->update_precio_cantidad ();
	}
